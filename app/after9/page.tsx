...
const questions: Question[] = [
  ...
  { id: 8, block: "Блок А", topic: "Сравнение чисел", prompt: "Расположите числа в порядке возрастания:", expression: "−√10;   −3,1;   −19/6", type: "order" },
  { id: 9, block: "Блок А", topic: "Чтение таблицы", prompt: "Ответьте на вопросы по таблице: 1. В какое время температура была наименьшей? 2. На сколько градусов температура повысилась с этого момента до 16:00?", type: "table" },
  { id: 10, block: "Блок А", topic: "Теорема Пифагора и площадь прямоугольника", prompt: "В прямоугольнике диагональ равна 13, а одна из сторон равна 5. Найдите площадь прямоугольника.", type: "number", diagram: "rectangle" },
  { id: 11, block: "Блок Б", topic: "Формулы сокращённого умножения", prompt: "Разложите на множители:", expression: "9x² − 24x + 16", type: "factor" },
  { id: 12, block: "Блок Б", topic: "Алгебраические дроби", prompt: "Сократите дробь и укажите допустимые значения переменной:", expression: "(x² − 9) / (x² − 3x)", type: "fraction" },
  { id: 13, block: "Блок Б", topic: "Квадратное уравнение", prompt: "Решите уравнение:", expression: "2x² − 7x + 3 = 0", type: "roots" },
  { id: 14, block: "Блок Б", topic: "Система уравнений", prompt: "Решите систему и найдите все пары (x; y):", expression: "x + y = 7,\nxy = 10.", type: "pairs" },
  { id: 15, block: "Блок Б", topic: "Квадратное неравенство", prompt: "Решите неравенство:", expression: "(x − 4)(x + 1) ≤ 0", type: "interval" },
  { id: 16, block: "Блок Б", topic: "Квадратичная функция", prompt: "Для функции y = x² − 4x + 3 укажите направление ветвей, нули, абсциссу вершины и выберите график.", type: "quadratic" },
  { id: 17, block: "Блок Б", topic: "Арифметическая прогрессия", prompt: "Первый член арифметической прогрессии равен −7, а разность равна 4. Найдите десятый член и сумму первых десяти членов.", type: "double" },
  { id: 18, block: "Блок Б", topic: "Задачи на движение", prompt: "Расстояние между городами равно 240 км. Автомобиль проехал первую половину пути со скоростью 60 км/ч, а вторую — со скоростью 80 км/ч. Сколько часов заняла вся поездка?", type: "number" },
  { id: 19, block: "Блок Б", topic: "Вероятность и статистика", prompt: "На карточках записаны числа: 2, 3, 3, 5, 7, 7, 7, 8. Найдите среднее арифметическое, медиану и вероятность случайно выбрать карточку с числом 7.", type: "triple" },
  { id: 20, block: "Блок Б", topic: "Хорды окружности", prompt: "Хорды AB и CD пересекаются в точке E внутри окружности. Известно: AE = 3, EB = 8, CE = 4. Найдите ED.", type: "number", diagram: "circle-chords" },
  { id: 21, block: "Блок В", topic: "Дробно-рациональные уравнения", prompt: "Решите уравнение:", expression: "3/(x − 2) − 2/(x + 2) = 14/(x² − 4)", type: "manual1" },
  { id: 22, block: "Блок В", topic: "Исследование квадратичной функции", prompt: "Дана функция y = −x² + 6x − 5. Найдите координаты вершины, наибольшее значение, решите неравенство −x² + 6x − 5 ≥ 0 и постройте схематичный график.", type: "manual3" },
  { id: 23, block: "Блок В", topic: "Задачи на совместную работу", prompt: "Две трубы вместе наполняют резервуар за 4 часа. Первая труба может наполнить резервуар самостоятельно на 6 часов быстрее второй. За сколько часов каждая труба наполнит резервуар отдельно?", type: "manual2" },
  { id: 24, block: "Блок В", topic: "Геометрическое доказательство", prompt: "В равнобедренной трапеции диагональ является биссектрисой острого угла. 1. Докажите, что боковая сторона равна меньшему основанию. 2. Найдите периметр, если основания равны 6 и 14.", type: "manualProof", diagram: "isosceles-trapezoid" },
];
...
<svg className="after9-svg" viewBox="0 0 430 270" role="img" aria-label="Геометрическая схема">
        ...
        {kind === "isosceles-trapezoid" && (
          <>
            ...
            <path d="M86 220 A42 42 0 0 0 80 199" className="angle-arc" />
            <path d="M77 201 A37 37 0 0 0 64 187" className="angle-arc" />
            <line x1="82" y1="148" x2="96" y2="156" className="tick" />
            <line x1="334" y1="156" x2="348" y2="148" className="tick" />
            <path d="M178 53 l10 7 -10 7 M207 53 l10 7 -10 7" className="parallel-mark" />
            <path d="M178 213 l10 7 -10 7 M207 213 l10 7 -10 7" className="parallel-mark" />
...
export default function AfterGradeNineDiagnostic() {
  ...
  const reportText = () => {
    ...
    const reference = skills.filter((item) => item.correct && item.reference).map((item) => item.name);
    const repeat = skills.filter((item) => !item.correct).map((item) => item.name);
    return [
      "Диагностика “Что повторить после 9 класса?”", `Ученик: ${name}`, `Дальнейшее обучение: ${destination}`,
      ...
      `Со справочными материалами: ${questions.filter((q) => answer(q.id).reference).length}`,
      ...
      `Получилось со справочными материалами:\n${reference.length ? reference.map((item) => `— ${item}`).join("\n") : "—"}`,
  ...
  const download = () => {
    const rows = questions.map((q) => {
      ...
      const content = [a.value && `Ответ: ${escapeHtml(a.value)}`, a.extra && `Дополнительный ответ: ${escapeHtml(a.extra)}`,
        ...
        a.explanation && `Ход решения: ${escapeHtml(a.explanation)}`, a.reference && "Использованы справочные материалы",
      ...
      <main className="test-shell oge-page after9-page">
        ...
        <section className="test-wrap">
          ...
          <article className="question-card after9-question">
            ...
            <label className="reference-check"><input type="checkbox" checked={stored.reference} onChange={(e)=>setAnswers((prev)=>({...prev,[question.id]:{...(prev[question.id]||blank()),reference:e.target.checked}}))}/><span>Использовал(а) справочные материалы</span></label>
...
if (screen === "bridgeC") {
  ...
  if (screen === "result") {
    ...
    return <main className="result-page after9-page"><header className="compact-header result-header"><a className="brand" href="/"><span className="brand-mark">∿</span><span>Математика без стресса</span></a><button className="text-button" onClick={restart}>Пройти ещё раз</button></header><section className="result-hero"><div className="score-orbit"><strong>{scoreA+scoreB}</strong><span>из 20</span></div><div><p className="kicker">Предварительный результат</p><h1>{name}, вот твой результат</h1><p>{conclusion}</p><small>Дальнейшее обучение: {destination}</small></div></section><section className="result-section result-stats oge-stats"><article><strong>{scoreA+scoreB}/20</strong><span>автоматическая часть</span></article><article><strong>{scoreA}/10</strong><span>блок А</span></article><article><strong>{scoreB}/10</strong><span>блок Б</span></article><article><strong>{questions.filter(q=>answer(q.id).reference).length}</strong><span>со справочными материалами</span></article><article><strong>{questions.filter(q=>answer(q.id).dontKnow).length}</strong><span>«не знаю»</span></article><article><strong>Ожидает</strong><span>проверка блока В</span></article></section><section className="result-section three-topic-panels oge-topic-panels">{self.length>0&&<article className="topic-panel strong-panel"><p className="kicker">Без подсказок</p><h2>Получилось самостоятельно</h2><div className="topic-tags">{self.map(s=><span key={s.key}>✓ {s.name}</span>)}</div></article>}{reference.length>0&&<article className="topic-panel repeat-panel"><p className="kicker">С опорой</p><h2>Получилось со справочными материалами</h2><div className="topic-tags">{reference.map(s=><span key={s.key}>{s.name}</span>)}</div></article>}{repeat.length>0&&<article className="topic-panel restore-panel"><p className="kicker">Для повторения</p><h2>Стоит повторить</h2><div className="topic-tags">{repeat.map(s=><span key={s.key}>{s.name}</span>)}</div></article>}</section><section className="result-section manual-block"><p className="kicker">Блок В</p><h2>Задания №21–24 ожидают проверки преподавателя</h2><div>{[21,22,23,24].map(id=><span key={id}>№{id} — {manualStatus(id)}</span>)}</div></section><section className="final-cta oge-final"><div><p className="kicker">Получить разбор и план повторения</p><h2>Отправь результат и фотографии решений Лере</h2><p>Я посмотрю не только ответы, но и ход работы, отмечу сильные стороны и темы для повторения, а затем предложу подходящий план.</p></div><div className="cta-actions oge-cta-actions"><button className="button secondary" onClick={()=>copyResult()}>Скопировать результат</button><button className="button secondary" onClick={download}>Скачать результат</button><a className="button primary" href={TELEGRAM_URL} target="_blank" rel="noreferrer" onClick={()=>copyResult("Результат скопирован. Вставь его в сообщение и прикрепи фотографии решений")}>Открыть Telegram Леры</a><button className="button secondary" onClick={restart}>Пройти ещё раз</button></div>{toast&&<p className="copy-toast" role="status">{toast}</p>}{copyFallback&&<div className="copy-fallback"><textarea readOnly value={copyFallback}/><button className="button secondary" onClick={()=>copyResult()}>Скопировать вручную</button></div>}</section></main>;
  }

  return <main className="home-page after9-page"><header className="site-header"><a className="brand" href="/"><span className="brand-mark">∿</span><span>Математика без стресса</span></a></header><section className="hero oge-hero"><div className="hero-copy"><div className="soft-pill">После основной школы</div><h1>Что повторить<br/><em>после 9 класса?</em></h1><p className="hero-lead">Проверим знания из программы 5–9 классов и определим, что стоит повторить перед 10 классом или колледжем.</p><div className="diagnostic-facts oge-facts"><span><b>24</b> задания</span><span>Три уровня сложности</span><span>Около 60 минут</span><span>Без школьной оценки</span><span>Разбор по темам</span><span>Блок В проверит преподаватель</span></div><label className="name-field"><span>Имя и фамилия ученика</span><input value={name} onChange={e=>setName(e.target.value)} placeholder="Введите имя и фамилию"/></label><fieldset className="destination-choice"><legend>Где ты продолжишь обучение?</legend>{["10 класс","колледж","пока не определился","другое"].map(v=><label key={v}><input type="radio" name="destination" checked={destination===v} onChange={()=>setDestination(v)}/>{v}</label>)}</fieldset><div className="start-guidance"><h2>Перед началом</h2><p className="paper-reminder">Приготовь несколько листов бумаги для вычислений и решений.</p><ul><li>Выполняй работу самостоятельно и по возможности за один подход.</li><li>Обычно нужно около 60 минут, но строгого ограничения нет.</li><li>Не используй калькулятор, интернет, учебник, конспекты и подсказки.</li><li>Если посмотрел формулу или правило, отметь справочные материалы в задании.</li><li>Не стирай неудачные попытки — они тоже помогают увидеть пробелы.</li><li>Если задание не получается, переходи дальше и вернись позже.</li><li>Блок В сложнее остальных. Если он пока не получается, это нормально.</li></ul><p>Эта работа нужна не для оценки, а для определения тем, которые стоит повторить перед дальнейшим изучением математики.</p><label className="consent-check"><input type="checkbox" checked={accepted} onChange={e=>setAccepted(e.target.checked)}/><span>Я прочитал(а) рекомендации и готов(а) начать</span></label></div><button className="button primary big" disabled={!name.trim()||!destination||!accepted} onClick={()=>setScreen("test")}>Начать диагностику →</button><p className="privacy-note">Имя, ответы и фотографии сохраняются только в этом браузере.</p></div><div className="doodle oge-doodle" aria-hidden="true"><span className="doodle-plus">10</span><span className="doodle-pi">x²</span><span className="doodle-frac"><b>5</b><i/><b>9</b></span><div className="doodle-paper"><div/><div/><div/><span>✓</span></div><span className="doodle-dot dot-one"/><span className="doodle-dot dot-two"/></div></section><footer><div className="brand"><span className="brand-mark">∿</span><span>Математика без стресса</span></div><p>Проверяем опорные знания, а не ставим оценку ♡</p></footer></main>;
}
